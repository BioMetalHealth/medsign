...
module BermudaSign
  ...
  PRODUCT_URL = ENV.fetch('PRODUCT_URL', 'https://www.bermudafranchise.com')
  PRODUCT_EMAIL_URL = ENV.fetch('PRODUCT_EMAIL_URL', PRODUCT_URL)
  PRODUCT_NAME = 'BermudaSign'
  DEFAULT_APP_URL = ENV.fetch('APP_URL', 'http://localhost:3000')
  GITHUB_URL = 'https://github.com/BermudaFranchise/BermudaSign'
  SUPPORT_EMAIL = ENV.fetch('SUPPORT_EMAIL', 'support@bermudafranchise.com')
  HOST = ENV.fetch('HOST', 'localhost')
  AATL_CERT_NAME = 'bermuda_sign_aatl'
  CONSOLE_URL = ''
  CLOUD_URL = ENV.fetch('CLOUD_URL', PRODUCT_URL)
  CDN_URL = if Rails.env.development?
              'http://localhost:3000'
            elsif ENV['MULTITENANT'] == 'true'
              "https://cdn.#{HOST}"
            else
              ENV.fetch('CDN_URL', 'http://localhost:3000')
            end
  ...
  def default_pkcs
    return if BermudaSign::CERTS['enabled'] == false

    @default_pkcs ||= GenerateCertificate.load_pkcs(BermudaSign::CERTS)
  end
  ...
  def fulltext_search?
    ...
    @fulltext_search =
      if SearchEntry.table_exists?
        BermudaSign.multitenant? || AccountConfig.exists?(key: :fulltext_search, value: true)
      else
        false
      end
  end
